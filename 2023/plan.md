...
Четвертая аксиома нужна для алгоритмов сортировки, но избыточна для более простых алгоритмов STL
(например она не нужна для `std::min` или `std::min_element`). Для таких алгоритмов достаточно
было бы потребовать от компаратора выполнения аксиом частичного порядка (первых трёх).
По всей видимости все алгоритмы требуют выполнения всех четырёх аксиом
чтобы не перегружать Стандарт математической терминологией.
